 ...
 * 通知 UI 模块
 * 
 * Manages the display and lifecycle of temporary notifications (toasts)
 * in the bottom-right corner of the application.
 * Supports different types (info, success, warning, error), auto-close, and manual close.
 * 管理应用程序右下角临时通知（toast）的显示和生命周期。
 * 支持不同类型（信息、成功、警告、错误）、自动关闭和手动关闭。
 */
...
/**
 * Gets or creates the main container element for notifications.
 * 获取或创建通知的主容器元素。
 * 
 * Ensures the container exists in the DOM, creating it if necessary.
 * 确保容器存在于 DOM 中，如果需要则创建它。
 * 
...
/**
 * Creates and displays a new notification element.
 * Handles positioning, animations (slide-in, push-up, fade-out), queue management, and user interactions.
 * 创建并显示新的通知元素。
 * 处理定位、动画（滑入、上推、淡出）、队列管理和用户交互。
 * 
 * @param message - The text content of the notification.
 *                - 通知的文本内容。
 * @param type - The category of the notification ('info', 'success', 'warning', 'error').
 *             - 通知的类别（'info', 'success', 'warning', 'error'）。
 * @param options - Optional configuration settings (duration, closable).
 *                - 可选的配置设置（持续时间、可关闭）。
 */
...
/**
 * Removes a specific notification element from the UI.
 * Handles the slide-out/fade-out animation and triggers the drop-down animation
 * for any notifications positioned above the removed one.
 * 从 UI 中移除指定的通知元素。
 * 处理滑出/淡出动画，并为位于被移除通知上方的任何通知触发下落动画。
 * 
