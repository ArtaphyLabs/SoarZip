 ...
 * 设置面板 UI 模块
 * 
 * Manages the creation, display, and interaction logic for the application settings panel.
 * Includes sidebar navigation for different setting categories (Appearance, General, Advanced)
 * and handles applying settings like theme changes.
 * 管理应用程序设置面板的创建、显示和交互逻辑。
 * 包括不同设置类别（外观、常规、高级）的侧边栏导航，
 * 并处理应用设置（如主题更改）。
 */
...
/**
 * Handles theme selection changes initiated from the settings panel.
 ...
 * 显示设置面板。
 * 如果需要，创建面板，将其和覆盖层添加到DOM中，并触发展开动画。
 */
