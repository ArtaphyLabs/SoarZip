 ...
 * 解压服务模块
 * 
 * Orchestrates the process of extracting files from an archive,
 * including showing the confirmation dialog and calling the backend.
 * 协调从压缩包中提取文件的过程，
 * 包括显示确认对话框和调用后端。
 */
 ...
 *
 * @param filesToExtractOverride - Optional array of file paths to extract, overriding the current selection.
 *                               - 可选的要解压的文件路径数组，覆盖当前选择。
 */
 ...
 *
 * @param destination - The destination folder path selected by the user.
 *                    - 用户选择的目标文件夹路径。
 * @param filesToExtract - Array of file paths relative to the archive root to extract.
 *                       - 相对于压缩包根目录的要解压的文件路径数组。
 */
